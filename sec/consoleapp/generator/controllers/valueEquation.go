...
func (d *GenValueEquation) Generate(base *BaseController) {
	...
	e = d.generateValueEquation(2014, "2110")
	if e != nil {
		tk.Println(e)
	}
...
func (d *GenValueEquation) generateValueEquation(year int, plantCode string) error {
	...
	if len(plants) > 0 {
		...
		for _, appendix := range generationAppendix {
			...
			// UnitInfos
			query := []*dbox.Filter{}
			query = append(query, dbox.Eq("Plant", plantCode))
			query = append(query, dbox.Eq("Year", year))
			query = append(query, dbox.Eq("Unit", unit))
			csr, e = c.NewQuery().From(new(UnitInfos).TableName()).Where(query...).Cursor(nil)
			unitInfos := []UnitInfos{}
			...
			if len(unitInfos) > 0 {
				...
				// FuelTransport
				query = append(query, dbox.Eq("Plant", plantCode))
				query = append(query, dbox.Gte("year", year))
				csr, e = c.NewQuery().From(new(FuelTransport).TableName()).Where(query...).Cursor(nil)
				fuelTransport := new(FuelTransport)
				e = csr.Fetch(&fuelTransport, 0, false)
				csr.Close()
				...
				dieselConsumptions = crowd.From(&unitInfos).Where(func(x interface{}) interface{} {
					...
					return x.(UnitInfos).PrimaryFuelConsumedCost
				}).Exec().Result.Sum
				...
				dieselConsumptionsBackup := crowd.From(&unitInfos).Where(func(x interface{}) interface{} {
					...
					return x.(UnitInfos).BackupFuelConsumedCost
				}).Exec().Result.Sum
				...
				query = append(query, dbox.Eq("Plant", plantCode))
				query = append(query, dbox.Eq("Unit", unit))
				query = append(query, dbox.And(dbox.Gte("ConsolidatedDate", yearStart), dbox.Lt("ConsolidatedDate", yearEnd)))
				...
				query = append(query, dbox.Eq("Plant", plantCode))
				query = append(query, dbox.Eq("Unit", unit))
				query = append(query, dbox.And(dbox.Gte("DatePerformed", yearStart), dbox.Lt("DatePerformed", yearEnd)))
				csr, e = c.NewQuery().From(new(PreventiveMaintenance).TableName()).Where(query...).Cursor(nil)
				preventiveMaintenance := []PreventiveMaintenance{}
				e = csr.Fetch(&preventiveMaintenance, 0, false)
				csr.Close()
				...
				query = append(query, dbox.Eq("Plant", plantCode))
				query = append(query, dbox.Eq("Unit", unit))
				query = append(query, dbox.And(dbox.Gte("StartDate", yearStart), dbox.Lt("StartDate", yearEnd)))
...
/*func (d *GenValueEquation) generateValueEquation(Year int, Plant string) error {
	...
	query = append(query[0:0], dbox.Eq("Plant", Plant))
	query = append(query, dbox.And(dbox.Gte("ScheduledStart", yearStart), dbox.Lt("ScheduledStart", yearEnd)))
