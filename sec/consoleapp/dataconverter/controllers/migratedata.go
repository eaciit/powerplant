...
func (m *MigrateData) DoValueEquation() error {
	...
	for _, val := range result {
		...
		*/
		...
		var e error
		for {
			id, e = m.InsertOut(val, new(ValueEquation))
			if e == nil {
				break
			} else {
...
func (m *MigrateData) DoGenerateUnitPower() error {
	tStart := time.Now()
	tk.Println("Starting DoGenerateUnitPower..")
	mod := new(UnitPower)
	...
	for _, val := range result {
		for {
			_, e := m.InsertOut(val, new(UnitPower))
			if e == nil {
				break
			} else {
				m.SqlCtx.Connection.Connect()
